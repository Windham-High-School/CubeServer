...
class EnvConfig:
    ...
    CS_DEV: bool = __bool_from_str(environ.get("CS_DEV", "FALSE"))
    CS_RELEASE: str = environ.get("CS_RELEASE", "dev" if CS_DEV else "")
    CS_DEVCCONTAINER: bool = __bool_from_str(environ.get("CS_DEVCONTAINER", "FALSE"))
    ...
    # GUnicorn parameters
    CS_APP_WORKER_TIMEOUT: int = __int_from_str(
        environ.get("CS_APP_WORKER_TIMEOUT", "90")
    )
    CS_APP_TIMEOUT: int = __int_from_str(environ.get("CS_APP_TIMEOUT", "45"))
    CS_APP_PRELOAD: int = __bool_from_str(environ.get("CS_APP_PRELOAD", "TRUE"))

    CS_API_WORKER_TIMEOUT: int = __int_from_str(
        environ.get("CS_API_WORKER_TIMEOUT", "90")
    )
    CS_API_TIMEOUT: int = __int_from_str(environ.get("CS_API_TIMEOUT", "45"))
    CS_API_PRELOAD: int = __bool_from_str(environ.get("CS_API_PRELOAD", "TRUE"))

    CS_MAX_UPLOAD_SIZE: int = __int_from_str(environ.get("CS_MAX_UPLOAD_SIZE", "32768"))
    ...
    CS_API_PORT: int = __int_from_str(environ.get("CS_API_PORT", "8081"))
