...
from cubeserver_common.models.utils.modelutils import PyMongoModel, EncodableCodec, Encodable, BSON_TYPES
...
class MyModel(PyMongoModel):
    def __init__(self, foo: str = '', bar: int = 0, baz: list = [], qux: MyEnum = MyEnum.FOO):
        super().__init__()
        self.foo = foo
        self.bar = bar
        self.baz = baz
