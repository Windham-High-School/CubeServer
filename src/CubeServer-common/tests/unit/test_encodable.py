...
class MyEncodable(Encodable):
    def __init__(self, foo: str, bar: int, baz: Optional[List[str]], qux: ObjectId, enum: MyEnum):
        self.foo = foo
        self.bar = bar
        self.baz = baz
        self.qux = qux
    ...
    def test_encodable(cls) -> 'MyEncodable':
        return MyEncodable(foo='foo', bar=42, baz=['baz'], qux=ObjectId(), enum=MyEnum.FOO)
...
def test_encodable_codec():
    ...
    assert EncodableCodec(MyEncodable).transform_python(
