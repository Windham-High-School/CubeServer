...
class RegistrationForm(FlaskForm):
    ...
    team_name = StringField(
        ...
        validators=[
            Length(
                min=1,
                max=config.DynamicConfig["System"]["Max Team Length"],
                message=f"Please provide a team name under {config.DynamicConfig['System']['Max Team Length']} characters.",
            )
        ],
    )

    _members_message = "Must have at least 2 members in a team."
    _emails_message = "Who doesn't have an email? Come on. Pony up."
    ...
    def validate_email1(_, field):
        ...
        if field.data and not field.data.endswith(
            config.DynamicConfig["Competition"]["Registration Email Domain"]
        ):
            raise ValidationError(
                "All emails provided must be from "
                f"{config.DynamicConfig['Competition']['Registration Email Domain']}"
            )
