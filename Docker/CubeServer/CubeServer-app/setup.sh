...
openssl x509 -in /etc/ssl/api_cert/server.pem -fingerprint -sha1 -noout | cut -d "=" -f2 > /etc/ssl/api_cert/sha1_fingerprint.txt
echo "Generating SHA256 Fingerprint for client constants code generation..."
openssl x509 -in /etc/ssl/api_cert/server.pem -fingerprint -sha256 -noout | cut -d "=" -f2 > /etc/ssl/api_cert/sha256_fingerprint.txt
